...
if ($_SERVER["REQUEST_METHOD"] == "POST") {// check if the form has been submitted using the post method
    // Retrieve form data (user input) and handle case where they are not set
     $firstname = isset($_POST["firstName"])? $_POST["firstName"]:"" ;// if the name is not set then the emplty string willbe asign
    ...
    if($confirm === $password){
        $sql = "INSERT INTO signupss (firstname, lastname, email, password_hash, isactive)
        VALUES ('$firstname', '$lastname', '$email', '$hashedPassword', 'N')";

        if ($conn->query($sql) === TRUE) {
            echo '<script>alert( "Signup successful!");</script>' ;
            header("location: C_login.php");//Redirects the user to the login page Redirects the user to the login page  
        } else {
...
<head>
    ...
    <link rel="stylesheet" href="./../assets2/C_signup.css">
...
<body>
    ...
    <div class="myform">
        <h1>SignUp</h1>
        <form id="signup-form" action="C_signup.php" method="post">
        ...
        <p class="login-link">Already have an account? <a href="C_login.php" style="color: blue;">Log In</a></p>
    </form>
</div>
<div class="image">
  ...
  <script>
    ...
    function isStrongPassword(password) {
        ...
        //The return statement: The function returns true if the password meets all the specified criteria (length, uppercase, lowercase, and numeric characters), and false otherwise.
    }
</script>
</body>
